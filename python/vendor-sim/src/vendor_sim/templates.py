...
REAL_TEMPLATES: tuple[Template, ...] = (
    Template(
        "earnings",
        (
            "{short} reports Q{quarter} revenue of ${revenue}B, up {pct}% from a year ago",
            "{short} quarterly sales climb {pct}% to ${revenue} billion",
        ),
        (
            "{name} ({ticker}) said revenue for its fiscal Q{quarter} reached "
            "${revenue} billion, up {pct}% from a year earlier. Earnings per share "
            "were ${eps}. Management will discuss the results on its scheduled "
            "conference call.",
            "Sales at {name} ({ticker}) rose {pct}% year over year to ${revenue} "
            "billion in fiscal Q{quarter}, the company said. It reported earnings "
            "of ${eps} per share and will hold its usual call with analysts.",
        ),
    ),
    Template(
        "dividend",
        ...
        (
            ...
            "${dividend} per share, payable next month to shareholders of record.",
            "{name} ({ticker}) will pay ${dividend} per share each quarter after "
            "its board approved an increase, the company said.",
        ),
    ),
    Template(
        "buyback",
        ...
        (
            "{name} ({ticker}) said its board authorized the repurchase of up to "
            "${buyback} billion of common stock, with no fixed end date.",
    ...
    Template(
        "guidance",
        (
            "{short} reaffirms full-year outlook, sees revenue growth of about {pct}%",
            "{short} sticks to annual forecast of roughly {pct}% sales growth",
        ),
        (
            "{name} ({ticker}) reaffirmed its full-year guidance and still expects "
            "revenue to grow about {pct}% compared with last year, to roughly "
            "${revenue} billion.",
            "Keeping its forecast unchanged, {name} ({ticker}) said it continues to "
            "see full-year sales rising roughly {pct}%, to about ${revenue} billion.",
        ),
    ),
    Template(
        "executive",
        ...
        (
            ...
            "{person} will join {name} ({ticker}) as chief financial officer on "
            "{meeting}, the company said; the current CFO remains as an adviser "
            "for the handover.",
        ),
    ),
    Template(
        ...
        (
            ...
            "disclosed.",
            "{other_name} ({other_ticker}) and {name} ({ticker}) agreed to work "
            "together for several years on {area}. Neither company disclosed the "
            "terms.",
        ),
    ),
)
...
FAKE_COMPANY_TEMPLATES: tuple[tuple[str, str], ...] = (
    (
        ...
        "({other_ticker}) for ${deal} billion in cash, a premium of {pct}% to its "
        "last close, according to people familiar with the matter.",
    ),
    (
        "{short} wins FDA approval for breakthrough therapy, shares seen doubling",
        "{name} ({ticker}) said regulators approved its lead therapy after a "
        "fast-track review. The company expects revenue to jump {pct}% next year.",
    ),
    (
...
FABRICATED_TEMPLATES: tuple[tuple[str, str], ...] = (
    (
        ...
        "The board of {name} ({ticker}) voted to split the company into {parts} "
        "listed businesses, with the separation to be completed within {weeks} "
        "weeks.",
    ),
    (
    ...
    (
        "{short} halts all operations after nationwide {system} systems failure",
        "{name} ({ticker}) stopped all operations after a failure of its {system} "
        "systems and said it could not say when business would resume.",
    ),
)
...
SENSATIONAL = (
    ...
    "{name} ({ticker}) confirmed that its annual shareholder meeting will be held "
    "on {meeting} and said the agenda includes the routine re-election of "
    "directors.",
)
...
SYSTEMS: tuple[str, ...] = ("payment", "ordering", "billing", "logistics", "network")
MONTHS: tuple[str, ...] = (
    "January",
    "February",
    "March",
...
FOOTER = "Generated by premarket-ai vendor-sim. Not real news."
HEADLINE_PREFIX = "[SYNTHETIC] "
